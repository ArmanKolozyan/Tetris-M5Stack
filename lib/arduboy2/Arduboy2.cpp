...
static bool frontBuffer[SCREEN_WIDTH * SCREEN_HEIGHT];
static bool backBuffer[SCREEN_WIDTH * SCREEN_HEIGHT];
...
static void displayScreen(void *mysprite)
{
  for (;;)
  {
    ...
    {
      memcpy(backBuffer, frontBuffer, SCREEN_HEIGHT*SCREEN_WIDTH);
...
//{ //if exclude this 4 parts screen devision and process all the big oBuffer, EPS8266 resets (
      ...
      for (xPos = 0; xPos < WIDTH; xPos++)
      {
        ...
        {
          if (!(yPos % 8))
            currentDataByte = sBuffer[xPos + (yPos/8) * WIDTH];
          xDst = xPos;
          yDst = yPos;
          loc = xDst + yDst * WIDTH;
